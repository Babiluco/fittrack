...
const AUTH = (function(){
  ...
  async function client(){
    if(typeof SupabaseClient.ensureReady === 'function'){
      await SupabaseClient.ensureReady();
    }
  ...
  function onAuthStateChange(callback){
    let cancelled = false;
    SupabaseClient.ensureReady().then(()=>{
      if(cancelled) return;
      const supabase = SupabaseClient.getClient();
      if(!supabase) return;
    ...
    return {unsubscribe(){ cancelled = true; if(authListener) authListener.unsubscribe(); }};
  }
